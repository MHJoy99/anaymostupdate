...
def start(update, context):
    ...
    buttons.buildbutton("Request Group", "https://t.me/MotionPicturerequest2")
    reply_markup = InlineKeyboardMarkup(buttons.build_menu(2))
    if CustomFilters.authorized_user(update) or CustomFilters.authorized_chat(update):
        start_string = f'''
This bot is for personal usage, only for MOTION PICTURE Admins!
Type /{BotCommands.HelpCommand} to get a list of available commands
'''
        sendMarkup(start_string, context.bot, update.message, reply_markup)
    else:
        sendMarkup('U Cant Use, Ask MOTION PICTURE Admins For Help', context.bot, update.message, reply_markup)

def restart(update, context):
    restart_message = sendMessage("Restart Kortesi...", context.bot, update.message)
    if Interval:
        Interval[0].cancel()
    clean_all()
